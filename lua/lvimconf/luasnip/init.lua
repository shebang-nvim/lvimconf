...
--- Filetype add snippets
---@param ft string: the filetype as string
---@param opts table|nil: FUTURE
M.load_snippets = function(ft, opts)
  _ = opts
  if not ft or ft == "" then
    return
  end
  log:debug(string.format("loading luasnips for '%s'", ft))
  local newsnips = require("lvimconf.luasnip.ft.lua")

  ls.add_snippets(ft, make(newsnips))
end

--- Filetype root handler
---@param event AutocmdEvent: the triggered auto command event
---@param opts table|nil: FUTURE
M.filetype = function(event, opts)
  _ = opts
  if not event then
    return
  end
  local ft = event.match
  log:debug("luasnip filetype root handler", event)
  M.load_snippets(ft)
end
...
M.setup = function(opts)
  ...
  -- Delegate to filetype handler
  vim.api.nvim_create_autocmd("FileType", {
    pattern = opts.enabled_ft,
    group = utils.augroup "filetype_handler",
    callback = require('lvimconf.luasnip').filetype
  })
end
...
vim.keymap.set("n", "<Space>bs", "<cmd>source ~/.config/lvim/lua/lvimconf/luasnip/init.lua<CR>", {desc = "Reload luasnip"})
return M
